...
class TestDataIngestion:
    def test_kafka_ingest_and_qdrant_store(self):
        subprocess.run(
            [os.path.join(".venv", "Scripts", "python.exe"), "scripts/01_ingest_to_kafka.py"],
            check=True,
        )
...
class TestObservability:
    ...
    def test_grafana_dashboard_accessible(self):
        resp = requests.get("http://localhost:3000/api/health", auth=("admin", "admin"), timeout=10)
        assert resp.status_code == 200


class TestFailurePath:
    def test_invalid_request_returns_422(self):
        resp = requests.post(f"{BASE_URL}/api/v1/chat", json={}, timeout=10)
